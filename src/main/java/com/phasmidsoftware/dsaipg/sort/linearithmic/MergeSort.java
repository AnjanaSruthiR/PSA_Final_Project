...
public class MergeSort<X extends Comparable<X>> extends SortWithComparableHelper<X> {
    ...
    private void sort(X[] a, X[] aux, int from, int to) {
        ...
        // Copy to aux array if not already using it
        if (!noCopy) {
            for (int k = from; k < to; k++) {
                helper.copy(a[k], aux, k);
            }
            merge(aux, a, from, mid, to); // merge from aux back into a

        } else {
            merge(a, aux, from, mid, to); // merge from a into aux
            for (int k = from; k < to; k++) {
    ...
    int i = from;
    int j = mid;
    // Cache the current elements from the left and right halves.
